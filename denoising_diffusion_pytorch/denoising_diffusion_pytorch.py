...
from denoising_diffusion_pytorch.datasets import Pingjun, Pingjun_cond
...
class Unet_conditional(Module):
    def __init__(
        ...
        # determine dimensions
        # NEW: Embedding layer for labels
        ...
        # NEW: Modify the initial convolution to accept additional label embedding channels
        # Input channels will be: channels (image channels) + label embedding dimension
        self.init_conv = nn.Conv2d(input_channels + embed_dim, init_dim, 7, padding=3)
    ...
    def forward(self, x, time, label, x_self_cond=None):
        ...
        # NEW: Embed the label and expand it to match the spatial dimensions of the image
        label_embed = (
            self.label_embedding(label).unsqueeze(2).unsqueeze(3)
        )  # Shape: (batch_size, label_embed_dim, 1, 1)
        label_embed = label_embed.expand(
            -1, -1, x.shape[2], x.shape[3]
        )  # Expand to match image H, W

        # NEW: Concatenate the image with the embedded label along the channel dimension
        x = torch.cat(
            (x, label_embed), dim=1
        )  # Now input has (channels + label_embed_dim) channels
...
class GaussianDiffusion(Module):
    ...
    def forward(self, img, *args, **kwargs):
        ...
        return self.p_losses(img, t, None, *args, **kwargs)


class GaussianDiffusion_conditional(Module):
    def __init__(
        self,
        model,
        *,
    ...
    @torch.inference_mode()
    def ddim_sample(self, shape, labels, return_all_timesteps=False):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = (
            shape[0],
            self.device,
            self.num_timesteps,
    ...
    @torch.inference_mode()
    def sample(self, batch_size=16, return_all_timesteps=False, labels = None):
        (h, w), channels = self.image_size, self.channels
        sample_fn = (
            self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        )
        ...
        if labels is None:
            #labels = torch.randint(0, 1, (batch_size,), device=self.device)
            
            #labels = [i for i in range(5) for _ in range(5)]
    ...
    def p_losses(self, x_start, t, label, noise=None, offset_noise_strength=None):
        b, c, h, w = x_start.shape
...
def get_sampler_for_balanced_classes(dataset):
    class_sample_count = np.array([3253, 1495, 2174, 1086, 251])
...
class Trainer:
    def __init__(
        ...
        *,
        ...
        self.ds = Pingjun_cond(self.image_size, binary_classification=True)

        assert (
            len(self.ds) >= 100
        ), "you should have at least 100 images in your folder. at least 10k images recommended"

        #sampler = get_sampler_for_balanced_classes(self.ds)
    ...
    def train(self, conditional_train=False):
        accelerator = self.accelerator
        device = accelerator.device
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        if conditional_train:
                            loss = self.model(data, label)
                        else:
                            loss = self.model(data, None)
